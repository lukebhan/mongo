...
TEST(Simple8b, EightSelectorLargeBase) {
    ...
    // 81E8F47A3D1E8F48
    uint128_t val = UInt128::MakeUint128FromString("8462480737302404222943232").value();
...
TEST(Simple8b, Selector8LargeSkipEncoding) {
    ...
    uint128_t val =
        UInt128::MakeUint128FromString("319014718988379809496913694467282698240").value();
...
TEST(Simple8b, Selector8LargeNibbleShift) {
    ...
    uint128_t val =
        UInt128::MakeUint128FromString("170141183460469231731687303715884105728").value();
    std::vector<boost::optional<uint128_t>> expectedInts = {val, val, val, val, val, val};
